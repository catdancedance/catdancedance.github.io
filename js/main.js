{
		...
		draw() {
			for (const link of this.links) {
				if (link.size) {
					...
					const s = Math.sin(a);
					// ---- shadow ----
					ctx.setTransform(c, s, -s, c, link.p0.x + link.size * 0.25, link.p0.y + link.size * 0.25);
					ctx.drawImage(
						link.shadow,
						-link.size * 0.5,
						-link.size * 0.5,
					...
					// ---- stroke ----
					ctx.setTransform(c, s, -s, c, link.p0.x, link.p0.y)
					ctx.drawImage(
						link.image,
						-link.size * 0.5,
						-link.size * 0.5,
	...
	Robot.Link = class Link {
		constructor(parent, p0, p1, p2, dist, size, light, force, disk) {
			// ---- cache strokes ----
	...
	const struct = {
		...
		links: [
			{ p0: 3, p1: 7, p2:body, size: 12, lum: 0.5 },  //left hand up
			{ p0: 1, p1: 3, p2:body,size: 24, lum: 0.5 },   //left hand down
			{ p0: 1, p1: 0, p2:head,size: 60, lum: 0.5, disk: 1 }, //head
			{ p0: 5, p1: 9, p2:paw,size: 16, lum: 0.5 },  //back leg down
			{ p0: 2, p1: 5, p2:paw,size: 32, lum: 0.5 },  //back leg up
			{ p0: 1, p1: 2, p2:body,size: 50, lum: 1 },    //body
			{ p0: 6, p1: 10, p2:paw,size: 16, lum: 1.5 },  //front leg down
			{ p0: 2, p1: 6, p2:paw,size: 32, lum: 1.5 },  //front leg up
			{ p0: 4, p1: 8, p2:body,size: 12, lum: 1.5 },  //right up
			{ p0: 1, p1: 4, p2:body,size: 24, lum: 1.5 }  //right down
		]
	};
	// ---- instanciate robots ----
	for (let i = 0; i < 1; i++) {
